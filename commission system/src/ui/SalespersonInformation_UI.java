...
public class SalespersonInformation_UI extends JFrame{
	/**
	 * 
	 */
...
//	int sid;
	...
	public SalespersonInformation_UI(int loginid) {
			...
			if (e.getSource() == jbuttonGetCommision) {
				try {
					...
					if(listSoldStatistics.get(k).getLocks() >= 1 && listSoldStatistics.get(k).getStocks() >= 1 && 
						...
						JOptionPane.showMessageDialog(null, str, "Wait Message!", JOptionPane.INFORMATION_MESSAGE);
						
						SalespersonInformation_DBOption saleop = new SalespersonInformation_DBOption();
						saleop.doUpdateFlagById(Integer.parseInt(id.substring(6)), listSoldStatistics.get(k).getCommision(), -1);
